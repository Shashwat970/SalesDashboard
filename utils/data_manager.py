...
def get_sales_data_for_product(product_db_id):
    ...
    return query_db(
        "SELECT sale_date, quantity_sold, revenue FROM sales_data WHERE product_id = ? ORDER BY sale_date ASC",
        (product_db_id,)
    )
    ...
    for index, row in df.iterrows():
        try:
            ...
            if current_product_db_id:
                ...
                cursor.execute(
                    "INSERT OR REPLACE INTO sales_data (product_id, sale_date, quantity_sold, revenue) VALUES (?, ?, ?, ?)",
                    (current_product_db_id, sale_date, quantity_sold, revenue)
                )
                inserted_rows += 1
...
def get_model_by_id(model_db_id):
    ...
    return query_db("SELECT m.*, p.name AS product_name FROM models m LEFT JOIN products p ON m.product_id = p.id WHERE m.id = ?", (model_db_id,), one=True)

def add_model(model_data):
    """Adds new model metadata to the database."""
    return execute_db(
        "INSERT INTO models (model_name, training_date, mae, rmse, r_squared, status, file_path, product_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def get_active_model(product_db_id=None):
    ...
    if product_db_id:
        ...
        model = query_db("SELECT * FROM models WHERE product_id = ? AND status = 'active'", (product_db_id,), one=True)
        if model:
            return model
...
def save_forecast(model_db_id, product_db_id, forecast_data):
    ...
    for row in forecast_data:
        try:
            cursor.execute(
                "INSERT OR REPLACE INTO forecasts (model_id, product_id, forecast_date, forecasted_quantity) VALUES (?, ?, ?, ?) ",
                (model_db_id, product_db_id, row['forecast_date'], row['forecasted_quantity'])
            )
        except Exception as e:
...
def get_forecasts_for_product(product_db_id, model_db_id=None):
    ...
    if model_db_id:
        return query_db(
            "SELECT forecast_date, forecasted_quantity FROM forecasts WHERE product_id = ? AND model_id = ? ORDER BY forecast_date ASC",
